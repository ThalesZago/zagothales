---
title: "Sales Serviços Aéreos"
description: "Public-facing marketing website for a Brazilian executive aviation company — built with Nuxt 3, TypeScript, Vue 3, and Bootstrap 5."
tags: ["Nuxt 3", "TypeScript", "Vue 3", "Bootstrap 5", "i18n", "SSR"]
...
Sales Serviços Aéreos is the corporate website for a Brazilian executive aviation and air charter company based at Campo de Marte Airport in São Paulo. The site allows visitors to browse the aircraft fleet, request flight quotes, read company news, and get contact and hangar information.
...
Dynamic content (blog posts, company history) is fetched from an external Strapi CMS at `admin.salesservicosaereos.com.br/api` using `useAsyncData` and `$fetch` inside Vue components. There are no custom Nitro server routes — Nuxt's SSR layer fetches and renders everything at request time, with SWR caching keeping the experience fast on repeat visits.
...
```
sales-servicos/
├── nuxt.config.ts        # Framework config, modules, runtime env
├── app.vue               # Root shell (NuxtLayout + NuxtPage)
├── i18n/
│   └── locales/          # pt-BR.json, en.json, es.json
└── src/                  # srcDir alias (@/*)
    ├── assets/           # Global CSS, SVG icons, images
    ├── components/       # Reusable UI components (carousels, modals, hero)
    ├── layouts/          # Default layout (header + slot + footer)
    ├── pages/            # File-based routes (blog, airplanes, contacts…)
    ├── sections/         # Page-level feature sections (quote form, newsletter)
    └── utils/            # Shared helpers (Brazilian states enum, etc.)
```
